...
__global__ void CalculateInvertedGrayScale(long int *d_red,long int *d_green,long int *d_blue,long int *d_T1,unsigned m,unsigned n) {
    unsigned x=blockIdx.x*blockDim.x +threadIdx.x; //Finding the index.
    if(x<m*n){                                     //index number Validation.
       unsigned k=(m-(x/n)-1)*n + x%n;             // Finding the mirror index for current index
        d_T1[k]=(d_red[x]+d_green[x]+d_blue[x])/3; // calculating the value and storing it in the mirror index element  
    }
}   


__global__ void CalculateThomasTransformation(long int *d_red,long int *d_green,long int *d_blue,long int *d_T2,unsigned m,unsigned n) {
    unsigned x=blockIdx.x*blockDim.x +threadIdx.x;   //Finding the index.
    if(x<m*n){                                       //index number Validation.
        d_T2[x]=d_red[x]/2+floor(sqrtf(d_green[x]))+d_blue[x]; // converting RBG values using thomas transformation.
    }
}
